...
namespace System.Reflection
{
    ...
    /// </summary>
    public abstract class AsyncDispatchProxy
    {
        protected AsyncDispatchProxy()
        {
        }
        ...
        protected abstract Task<T> InvokeTAsync<T>(MethodInfo? targetMethod, object?[]? args);
        protected abstract ValueTask InvokeValueAsync(MethodInfo? targetMethod, object?[]? args);
        protected abstract ValueTask<T> InvokeValueTAsync<T>(MethodInfo? targetMethod, object?[]? args);
        /// <summary>
        ...
        public static T Create<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.All)] T, [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicParameterlessConstructor)] TProxy>()
            where TProxy : AsyncDispatchProxy
        {
            return (T)AsyncDispatchProxyGenerator2.CreateProxyInstance(typeof(TProxy), typeof(T), "T", "TProxy");
        }
        ...
        public static object Create([DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.All)] Type interfaceType, [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicParameterlessConstructor)] Type proxyType)
        {
            ...
            {
                throw new ArgumentException(string.Format("The provided proxy type '{0}' must be derived from 'DispatchProxy'.", proxyType.Name), nameof(proxyType));
            }

            return AsyncDispatchProxyGenerator2.CreateProxyInstance(proxyType, interfaceType, "interfaceType", "proxyType");
        }
    }
}
