...
{
    internal static class AsyncDispatchProxyGenerator2
    {
        ...
        private static readonly MethodInfo s_dispatchProxyInvokeMethod = typeof(AsyncDispatchProxy).GetMethod("Invoke", BindingFlags.NonPublic | BindingFlags.Instance)!;
        private static readonly MethodInfo s_dispatchProxyInvokeAsyncMethod = typeof(AsyncDispatchProxy).GetMethod("InvokeAsync", BindingFlags.NonPublic | BindingFlags.Instance)!;
        private static readonly MethodInfo s_dispatchProxyInvokeTAsyncMethod = typeof(AsyncDispatchProxy).GetMethod("InvokeTAsync", BindingFlags.NonPublic | BindingFlags.Instance)!;
        private static readonly MethodInfo s_dispatchProxyInvokeValueAsyncMethod = typeof(AsyncDispatchProxy).GetMethod("InvokeValueAsync", BindingFlags.NonPublic | BindingFlags.Instance)!;
        private static readonly MethodInfo s_dispatchProxyInvokeValueTAsyncMethod = typeof(AsyncDispatchProxy).GetMethod("InvokeValueTAsync", BindingFlags.NonPublic | BindingFlags.Instance)!;
        ...
        private sealed class ProxyAssembly
        {
                ...
                string interfaceParameter, string proxyParameter)
            {
                ...
                {
                    throw new ArgumentException(string.Format("The type '{0}' must be an interface, not a class.", interfaceType.FullName), interfaceParameter);
                }

                // The base type cannot be sealed because the proxy needs to subclass it.
                if (baseType.IsSealed)
                {
                    throw new ArgumentException(string.Format("The base type '{0}' cannot be sealed.", baseType.FullName), proxyParameter);
                }

                // The base type cannot be abstract
                if (baseType.IsAbstract)
                {
                    throw new ArgumentException(string.Format("The base type '{0}' cannot be abstract.", baseType.FullName), proxyParameter);
                }

                // The base type must have a public default ctor
                if (baseType.GetConstructor(Type.EmptyTypes) == null)
                {
                    throw new ArgumentException(string.Format("The base type '{0}' must have a public parameterless constructor.", baseType.FullName), proxyParameter);
                }
...
#pragma warning restore IL2072
        ...
        private sealed class ProxyBuilder
        {
            ...
            private MethodBuilder AddMethodImpl(MethodInfo mi, int methodInfoIndex)
            {
                ...
                il.Emit(OpCodes.Ldloc, methodInfoLocal);
                argsArr.Load();
                var invokeMethod = s_dispatchProxyInvokeMethod;
                if (mi.ReturnType == typeof(Task))
                {
                    invokeMethod = s_dispatchProxyInvokeAsyncMethod;
                }
                else if (mi.ReturnType == typeof(ValueTask))
                {
                    invokeMethod = s_dispatchProxyInvokeValueAsyncMethod;
                }
                else if (IsGenericTask(mi.ReturnType))
                {
                    var returnTypes = mi.ReturnType.GetGenericArguments();
                    invokeMethod = s_dispatchProxyInvokeTAsyncMethod.MakeGenericMethod(returnTypes);
                }
                else if (IsGenericValueTask(mi.ReturnType))
                {
                    var returnTypes = mi.ReturnType.GetGenericArguments();
                    invokeMethod = s_dispatchProxyInvokeValueTAsyncMethod.MakeGenericMethod(returnTypes);
                }
